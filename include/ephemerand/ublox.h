...
using UbloxMessage = std::variant<UbloxMessage_Version, UbloxMessage_AlmanacMissing, UbloxMessage_AlmanacData>;
    ...
    void handleMessage(uint8_t message_class, uint8_t message_id, std::string &message) {
        if (message_class == MSG_CLASS_MON && message_id == MSG_ID_MON_VER) {
            ...
            if (message.size() == sizeof(ublox::AlmanacNoPayload)) {
                ublox::AlmanacNoPayload *alm = (ublox::AlmanacNoPayload *)message.data();
                output_queue.push_move(UbloxMessage_AlmanacMissing{alm->svprn});
            } else if (message.size() == sizeof(ublox::AlmanacWithPayload)) {
                ublox::AlmanacWithPayload *alm = (ublox::AlmanacWithPayload *)message.data();
            ...
            std::cerr << "Unknown ublox message type: " << message_class << " / " << message_id << std::endl;
        }
    }
